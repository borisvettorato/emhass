...
### The RC physics model can also learn coupling, not just ArxModel

The RC physics model (`rc-model-refit`/`rc-model-tune`) fits every room independently by default - it has no built-in notion of one room warming or cooling another. If you've declared a pair via `heatpump_room_coupled_neighbors` (the same field ArxModel and dispatch already use - nothing new to configure there), each room in the pair *also* gets its own dynamically-added coupling coefficient in the RC fit itself, alongside its usual envelope/solar/opening parameters - not just a manually-typed constant applied only at dispatch time.

This closes a real gap: two independently-fit rooms with an open, doorless connection (say a south-facing living room and a north-facing kitchen) can otherwise misattribute real cross-room heat flow to the wrong local parameter - e.g. a north-facing room's fit finding an implausible amount of its own "solar gain" that's actually delayed warmth arriving from its sun-exposed neighbor. Adding the coupling term lets each room's fit separate "my own physics" from "heat arriving from next door" - on real data this closed most of the accuracy gap a naive uncoupled fit had against ArxModel's own (already-coupled) result for the same rooms.

Same publish/opt-in split as ArxModel's own coupling (`rc_model_coupling_source`, default `"informational"`):

- **Published/logged only by default** - the RC-fitted, physically-scaled conductance for every declared pair is written to `data/rc_model_coupling.json`, purely for you to compare against your manually-entered `heatpump_room_coupling_conductance`; dispatch is unaffected.
- **Opt-in to real dispatch** with `rc_model_coupling_source: "auto_dispatch"` - the next optimization run uses the RC-fitted value for any pair it covers, falling back to the manual value for any pair it doesn't.
- If you turn on `auto_dispatch` for **both** `arx_model_coupling_source` and `rc_model_coupling_source` for the same pair, the RC-fitted value wins - it's the physically-native match for dispatch's own coupling equation (the same simple first-order conductance form), whereas ArxModel's is a converted linear-regression coefficient.

### Self-learning dispatch: letting the fitted model drive a room, not just forecast it

The Heat Pump tab has a global **"Advanced model driving dispatch"** select (`heatpump_dispatch_model`), with `arx_model` as one of its options. Setting it applies to every configured room at once - EMHASS doesn't support mixing ARX-driven and RC-driven dispatch across different rooms. Selecting `arx_model` is a statement of intent - "let the fitted ARX model actually dispatch every room once it's proven itself, room by room" - but whether a given room actually gets driven by it is still decided automatically, per room, on every `arx-model-refit`, never by the select alone:
...
### RC model refit/tune: learning each opening's own effect, dynamically

The live-dispatch behavior above (pause heating, add ventilation loss, boost door-coupling) is deliberately limited to ONE merged "is anything open right now" signal per room - a future opening can't be forecast, so dispatch never tries to reason about which *specific* opening will occur. The RC thermal model's own `rc-model-refit`/`rc-model-tune` fitting, by contrast, can look at a room's *entire* historical window at once, and learns a genuinely richer picture from it:

- **Configure `heatpump_room_opening_sensors`** (Rooms tab) with a comma-separated list of this room's opening entity_ids, of any length - one entry per door, window, or anything else with a binary_sensor open/closed state. Physics doesn't care whether an opening is a door or a window: each one gets its OWN independently-fitted ventilation-loss coefficient, PLUS a pairwise interaction coefficient for any two openings whose historical data actually shows them open at the same time often enough to fit a real coefficient from (a pair that never co-occurs is simply skipped, not fit to noise). This is genuinely dynamic - adding a 3rd, 4th, or Nth opening sensor to a room never needs a code change or a new config field, just one more entry in the list.
- The list's *first* entry keeps the "primary door" role described above (pause-heating boost + neighbor-coupling boost); every other entry is refit/tune-diagnostic-only - it improves the model's own historical accuracy and shows you which specific combination of openings matters most for that room, but (like the primary opening) never changes what live dispatch or an MPC forecast assumes for the future.
- **Adding a new opening sensor to an already-fitted room does not require a full refit.** As soon as `rc-model-refit`/`rc-model-tune` next runs, it detects the new sensor, freezes every already-known parameter (this room's core physics AND every already-fitted opening) at its previous value, and fits *only* the new opening's own coefficient(s) - a small, fast fit that by construction can't disturb anything that already worked. A scheduled full refit still runs on its own normal cadence and remains valuable (it can jointly re-balance everything, which the fast incremental path deliberately doesn't try to do) - the fast path is purely an extra, opportunistic shortcut triggered by a genuinely new sensor appearing in your config.
- `heatpump_room_window_sensors`/`heatpump_room_door_sensors`/`heatpump_room_door_sensors_2` (the fields this replaces) are still read, and still work, for a room that leaves `heatpump_room_opening_sensors` empty - concatenated in that order (door, then door 2, then window) into the same dynamic list, a one-time, in-memory-only migration with no config file rewrite needed.
