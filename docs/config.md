...
- `load_forecast_quantile_bias`: A blend factor that biases the load forecast toward a conservative P90 (high) estimate. Type: float. Valid range: `[0, 1]`. Default: `0.0` (pure P50, unchanged behaviour). A value of `1.0` uses the P90 estimate exclusively. Intermediate values blend linearly: `estimate = bias * P90 + (1 - bias) * P50`. Unlike the PV bias above (where conservative means less generation, P10), a conservative *load* estimate means more consumption, P90. The P90 is built via top-down temporal reconciliation: a historically-informed daily total is disaggregated back down to each timestep using the point forecast's own shape, so the reconciled timesteps sum to that daily total rather than each independently assuming its own worst case at once. Works with any `load_forecast_method`.
```{note} 
