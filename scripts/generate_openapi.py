...
SKIP = {"/", "/index", "/template", "/configuration", "/static/{filename}", "/thermal-comfort"}
...
def build_spec(routes: set | None = None) -> dict:
    ...
    paths = {
        ...
        "/room-schedule": {
            "get": {
                "summary": "Get the per-room weekly thermal comfort schedule",
                "responses": {
                    "200": {
                        "description": "Persisted schedule (or an empty default)",
                        **json_ct(
                            {
                                "type": "object",
                                "properties": {
                                    "weekSchedule": {"type": "object"},
                                    "presets": {"type": "object"},
                                },
                            }
                        ),
            ...
            "post": {
                "summary": "Save the per-room weekly thermal comfort schedule",
                "requestBody": json_ct(
                    {
                        "type": "object",
                        "properties": {
